...
OUT = "docs/technical_summary_v1.pdf"
IMG = "docs"
...
story.append(Paragraph("<i>Table 3: Data sources ingested into the RAG system. Papers expanded from 4 to 33 files.</i>", S["cap"]))
...
story.append(Paragraph("The FastAPI backend exposes six endpoints. The design follows REST conventions for resource retrieval (health, reactions) and uses Server-Sent Events for the chat endpoint to provide real-time progress updates as the LangGraph pipeline executes. Two additional endpoints support the AI-LNP Agent molecular analysis functionality via RDKit. CORS is enabled for all origins to support local development with the Angular dev server on a different port.", S["body"]))
...
], [0.7 * inch, 1.3 * inch, 1.6 * inch, 2.6 * inch])
story.append(Paragraph("<i>Table 7: API endpoint specifications. Added /analyze and /analyze-batch for RDKit integration.</i>", S["cap"]))
...
# ============================================================
story.append(Paragraph("7. Standalone & Desktop Applications", S["h1"]))
story.append(Paragraph("In addition to the Angular web frontend, the system provides two alternative deployment options: a standalone Dioxus WASM application for molecular analysis, and a native desktop application that embeds the full Angular frontend. These alternatives address different use cases\u2014the Dioxus standalone app provides a lightweight, single-purpose molecular property calculator, while the desktop app packages the complete web application as a native executable with no browser dependency.", S["body"]))

story.append(Paragraph("7.1 AI-LNP Agent (Dioxus Standalone)", S["h2"]))
story.append(Paragraph("The AI-LNP Agent is a standalone web application built with Dioxus 0.6 and compiled to WebAssembly. It provides a focused interface for molecular property analysis using RDKit via the backend API. Users can input SMILES strings and receive immediate feedback on drug-likeness (QED), synthetic accessibility (SA Score), and physicochemical properties (LogP, TPSA, molecular weight, H-bond donors/acceptors, rotatable bonds, ring count, heavy atoms). The application includes three pre-configured example molecules (DLin-MC3-DMA, ALC-0315-like, SM-102-like) and displays 2D molecular structures as SVG images.", S["body"]))
story.append(Paragraph("The Dioxus app is built using Trunk (a WASM bundler) and served via a simple Python HTTP server on port 8001. The entire application compiles to a single WASM binary (~200 KB gzipped) that runs entirely in the browser, making API calls to the FastAPI backend for RDKit computations. This architecture keeps the frontend lightweight while leveraging the existing Python/RDKit infrastructure for molecular analysis.", S["body"]))

story.append(Paragraph("<font face='Courier' size=8># Build and serve\ncd standalone\ntrunk build --release\npython serve.py  # Serves on http://localhost:8001</font>", styles["Code"]))

story.append(Paragraph("7.1.1 Molecular Property Scoring", S["h2"]))
story.append(Paragraph("The standalone app displays 10 molecular properties with color-coded scoring:", S["body"]))
tbl([
...
], [1.2 * inch, 1.8 * inch, 1.2 * inch, 2 * inch])
story.append(Paragraph("<i>Table 11: Molecular properties computed by the AI-LNP Agent.</i>", S["cap"]))

story.append(Paragraph("7.2 Desktop Application (wry + tao)", S["h2"]))
story.append(Paragraph("The desktop application is a native executable built with Rust using wry 0.47 (a cross-platform WebView library) and tao (a window management library). It embeds the production build of the Angular frontend at compile time using Rust's include_str! and include_bytes! macros, eliminating the need for external file dependencies. The entire application\u2014HTML, JavaScript, CSS, and assets\u2014is compiled into a single binary (~8 MB) that runs on Linux, macOS, and Windows without requiring a web browser or Node.js runtime.", S["body"]))
story.append(Paragraph("The desktop app uses a custom protocol handler (app://localhost/) to serve the embedded Angular files. When the user navigates to Angular routes (e.g., /reactions, /workflow), the protocol handler implements SPA fallback by serving index.html for all unmatched paths, ensuring client-side routing works correctly. The application opens in a 1280x820 window with the title '\ud83e\uddec Reactome LNP Agent' and connects to the FastAPI backend on localhost:8000.", S["body"]))

story.append(Paragraph("<font face='Courier' size=8># Build desktop app\ncd standalone-desktop\n./build.sh  # Compiles to target/release/lnp-desktop\n\n# Run (requires backend on port 8000)\n./target/release/lnp-desktop</font>", styles["Code"]))

story.append(Paragraph("7.2.1 Embedded Assets", S["h2"]))
story.append(Paragraph("The desktop build process copies the Angular production build into standalone-desktop/frontend/ and embeds four files:", S["body"]))
tbl([
...
], [1.5 * inch, 1 * inch, 1 * inch, 2.7 * inch])
story.append(Paragraph("<i>Table 12: Embedded assets in the desktop application.</i>", S["cap"]))

story.append(Paragraph("7.3 Deployment Comparison", S["h2"]))
tbl([
