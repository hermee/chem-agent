"""Generate technical_summary_v3.pdf — updated for dual-model architecture (Haiku 3.5 + Sonnet 4.5)."""
import os, re
...
OUT = "docs/technical_summary_v3.pdf"
IMG = "docs"
...
story.append(Paragraph("<i>Table 4: Node specifications. Haiku 3.5 handles 3 fast nodes; Sonnet 4.5 handles 5 expert + lead.</i>", S["cap"]))
...
story.append(Paragraph("Amazon Bedrock serves as the inference backbone. Version 3 introduces a dual-model architecture: Claude 3.5 Haiku for fast infrastructure tasks (router, query rewrite, reranking) and Claude Sonnet 4.5 for expert reasoning and the Lead Agent. Both are accessed through langchain-aws wrappers. Two LLM instances are configured: a standard instance (Sonnet 4.5, max_tokens=4096) for the Lead Agent and expert workers, and a fast instance (Haiku 3.5, max_tokens=1024) for the Router, Rewrite, and Reranking nodes. This tiered approach reduces cost by ~80% on high-frequency nodes with negligible quality impact.", S["body"]))
...
story.append(Paragraph("<i>Table 8: Bedrock foundation models. Dual-model architecture introduced in v3.</i>", S["cap"]))
...
# 11. CHANGELOG
# ============================================================
story.append(Paragraph("11. Changelog: v1 \u2192 v2 \u2192 v3", S["h1"]))

story.append(Paragraph("11.1 v2 \u2192 v3 Changes", S["h2"]))
tbl([
    ["Area", "v2", "v3"],
...
story.append(Paragraph("<i>Table 16: Summary of changes from v2 to v3.</i>", S["cap"]))

story.append(Paragraph("11.2 v1 \u2192 v2 Changes", S["h2"]))
tbl([
    ["Area", "v1", "v2"],
...
story.append(Paragraph("<i>Table 17: Summary of changes from v1 to v2.</i>", S["cap"]))
